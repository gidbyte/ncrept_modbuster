...
def setup():
	...
	macaddr = subprocess.getoutput("arp -n "+ nm-config.scada_ip +" | awk '/ether/ {print$3}'")
	
	newdata = filedata.replace(nm-config.scada_mac, macaddr)
...
#Write to Modbus Client MAC Variable
	...
	macaddr = subprocess.getoutput("arp -n "+ nm-config.modcli_ip +" | awk '/ether/ {print$3}'")
	
	newdata = filedata.replace(nm-config.modcli_mac, macaddr)
...
#Write to Modbus Port Variable
	...
	newdata = filedata.replace(nm-config.mod_port, 502)
